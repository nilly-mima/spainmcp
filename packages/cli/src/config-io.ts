...
import { ClientInfo, getOS } from "./clients.js"
...
function getMcpRemotePath(): string {
  const os = getOS()
  if (os === "windows") {
...
function buildEntry(apiKey: string) {
...
function addToClaudeConfig(configPath: string, apiKey: string): void {
  let config: Record<string, unknown> = {}
  if (fs.existsSync(configPath)) {
    config = JSON.parse(fs.readFileSync(configPath, "utf-8"))
  }
  if (!config.mcpServers) config.mcpServers = {}
  ;(config.mcpServers as Record<string, unknown>)[SERVER_NAME] = buildEntry(apiKey)
...
function addToMcpJson(configPath: string, apiKey: string): void {
  let config: Record<string, unknown> = {}
  if (fs.existsSync(configPath)) {
    config = JSON.parse(fs.readFileSync(configPath, "utf-8"))
  }
...
function addToVSCodeConfig(configPath: string, apiKey: string): void {
  let config: Record<string, unknown> = {}
  if (fs.existsSync(configPath)) {
    config = JSON.parse(fs.readFileSync(configPath, "utf-8"))
  }
  ...
  ;(mcp.servers as Record<string, unknown>)[SERVER_NAME] = buildEntry(apiKey)
  writeJson(configPath, config)
}

// ── Public API ────────────────────────────────────────────────────────────────

export function writeClientConfig(client: ClientInfo, apiKey: string): string {
  const os = getOS()
  const configPath = client.configPath[os]
  if (!configPath) throw new Error(`Client ${client.id} not supported on ${os}`)
...
export function removeClientConfig(client: ClientInfo): boolean {
  const os = getOS()
  const configPath = client.configPath[os]
  ...
  if (client.id === "vscode" || client.id === "vscode-insiders") {
    ...
    if (servers) delete servers[SERVER_NAME]
  }
...
export function isClientConfigured(client: ClientInfo): boolean {
  const os = getOS()
  const configPath = client.configPath[os]
  if (!configPath || !fs.existsSync(configPath)) return false
