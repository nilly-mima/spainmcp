#!/usr/bin/env node
/**
 * Backfill file_tree for github skills from github/gh-aw repo.
 * Same logic as import-openai-skills-filetree.mjs but different prefix.
 */
...
console.log('Fetching github/gh-aw tree...');
const treeRes = await fetch('https://api.github.com/repos/github/gh-aw/git/trees/main?recursive=1');
...
for (const [slug, entries] of bySlug) {
  ...
  try {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/skills_catalog?slug=eq.${slug}&author=eq.github`, {
      method: 'PATCH',
      headers: {
