...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    setNext(params.get('redirect') ?? params.get('next') ?? '/')
  }, [])
  const [state, setState] = useState<State>('idle')
  const [errorMsg, setErrorMsg] = useState('')

  const inputClass =
    'w-full px-4 py-2.5 rounded-xl text-sm text-stone-800 dark:text-stone-200 placeholder-stone-400 dark:placeholder-stone-500 outline-none focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-800 bg-white dark:bg-[var(--background)]'
  const btnClass =
