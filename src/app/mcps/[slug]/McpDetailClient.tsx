...
/* ── Icons ── */
const VerifiedIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-orange-500 shrink-0">
...
const ALL_CLIENTS: Record<string, ClientInfo> = {
  ...
  'claude':          { label: 'Claude',           color: '#C2400C', install: MCP_REMOTE },
  ...
  'highlight':       { label: 'Highlight',        color: '#EA580C', install: MCP_REMOTE },
  ...
  'claude-desktop':  { label: 'Claude Desktop',   color: '#C2400C', install: MCP_REMOTE },
}
...
function ToolItem({ name, description, parameters }: { name: string; description: string; parameters?: ToolParam[] }) {
  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ border: '1px solid #E8E2D9' }}>
      ...
      >
        <span className={`font-mono text-xs font-semibold tracking-wide uppercase ${open ? 'text-orange-600' : 'text-stone-700 dark:text-stone-300'}`}>
          {name}
        </span>
              ...
              {parameters.map(p => (
                <div key={p.name} className="flex flex-col gap-1 py-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {p.required && <span className="text-orange-500 text-xs font-bold">*</span>}
                    <span className="text-xs px-2 py-0.5 rounded-full bg-orange-50 border border-orange-200 text-orange-600 font-mono dark:bg-orange-950/30 dark:border-orange-800 dark:text-orange-400">
                      {p.type}
                    </span>
                  </div>
...
function ClientList() {
  ...
  if (selected) {
    ...
    return (
      <div className="flex flex-col gap-4 rounded-xl bg-white dark:bg-stone-900 p-4" style={{ border: '1px solid #E8E2D9' }}>
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg mb-1 bg-stone-50 dark:bg-stone-800" style={{ border: '1px solid #E8E2D9' }}>
        <SearchIcon />
      ...
      <div className="flex flex-col rounded-xl bg-white dark:bg-stone-900 px-3 max-h-72 overflow-y-auto" style={{ border: '1px solid #E8E2D9' }}>
...
function AgentPromptCard({ nombre, instalacion_npx }: { nombre: string; instalacion_npx: string }) {
  ...
  return (
    <div className="rounded-xl flex flex-col bg-white dark:bg-stone-900 overflow-hidden" style={{ border: '1px solid #E8E2D9' }}>
      {/* Header */}
      ...
      {/* Quote box */}
      <div className="mx-4 mb-4 mt-2 rounded-lg bg-white dark:bg-stone-800/40 px-4 py-3 text-sm text-stone-600 dark:text-stone-400 leading-relaxed" style={{ border: '1px solid #E8E2D9' }}>
        &#8220;Conecta a{' '}
...
function LatencyChart({ data }: { data: number[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
      ...
      <path d={smooth} fill="none" stroke="#F97316" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}
...
function PerformanceTab({ mcp }: { mcp: Mcp }) {
  ...
  const uptimeBars = Array.from({ length: 30 }, (_, i) => i < 4 ? 'grey' : i % 11 === 7 ? 'grey' : 'orange')
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,380px)_1fr] gap-4">
        ...
        {/* Tool table */}
        <div className="rounded-xl overflow-hidden bg-white dark:bg-stone-900" style={{ border: '1px solid #E8E2D9' }}>
          <table className="w-full text-xs">
            ...
            <tbody>
              {toolStats.map((t, i) => (
                <tr key={t.name} className={`border-b border-stone-50 dark:border-stone-800/30 ${i === 0 ? 'bg-orange-50/70 dark:bg-orange-950/20' : ''}`}>
        ...
        <div className="flex flex-col gap-4 h-full">
          <div className="rounded-xl p-4 bg-white dark:bg-stone-900 flex flex-col gap-3" style={{ border: '1px solid #E8E2D9' }}>
            ...
            <div className="flex gap-0.5">
              {uptimeBars.map((c, i) => (
                <div key={i} className="flex-1 h-8 rounded-sm" style={{ background: c === 'orange' ? '#F97316' : '#E2DDD6' }} />
              ))}
            </div>
          </div>
          <div className="rounded-xl p-4 bg-white dark:bg-stone-900 flex flex-col flex-1 min-h-0" style={{ border: '1px solid #E8E2D9' }}>
...
function UsageTab({ mcp }: { mcp: Mcp }) {
  const topClients = [
    { label: 'SpainMCP',    color: '#F97316', count: 14537 },
    { label: 'Claude Code', color: '#7C3AED', count: 7907 },
    { label: 'Claude.ai',   color: '#F97316', count: 4264 },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,300px)_1fr] gap-4">
        ...
        {/* Top Clients */}
        <div className="rounded-xl bg-white dark:bg-stone-900 p-5 flex flex-col gap-4 h-full" style={{ border: '1px solid #E8E2D9' }}>
              ...
              <div key={c.label} className="flex items-center gap-2.5">
                ...
                <div className="flex-1 flex flex-col gap-1 min-w-0">
                  ...
                  <div className="h-1 bg-stone-100 dark:bg-stone-800 rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-orange-500" style={{ width: `${(c.count / maxCount) * 100}%` }} />
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-xl bg-white dark:bg-stone-900 p-5 flex flex-col h-full" style={{ border: '1px solid #E8E2D9' }}>
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
            ...
            {gridVals.map(v => (
              <g key={v}>
                <line x1={PL} y1={toY(v)} x2={W - PR} y2={toY(v)} stroke="#E8E2D9" strokeWidth="1" strokeDasharray="4,4"/>
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => !t.soon && handleTabClick(t.id)}
              className={`flex items-center gap-2 px-4 py-3 text-sm border-b-2 -mb-px transition-colors ${
                tab === t.id
                  ? 'border-orange-500 text-stone-900 dark:text-stone-100 font-medium'
      ...
      {tab === 'overview' && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,660px)_1fr] gap-8">
                ...
                <div style={{ borderBottom: '1px solid #E8E2D9' }}>
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-1 pb-2 text-sm font-medium border-b-2 -mb-px transition-colors ${
                      capTab === 'tools' ? 'border-orange-500 text-orange-600' : 'border-transparent text-stone-500'
                    }`}
                  >
                    <ToolIcon size={12} />
                    Tools
          ...
          <div className="sticky top-6">
            <div className="rounded-xl p-5 flex flex-col gap-4" style={{ border: '1px solid #E8E2D9' }}>
              <h2 className="flex items-center gap-2 text-base font-semibold text-stone-800 dark:text-stone-200">
              ...
              <p className="text-xs text-stone-400 text-center pt-1">
                ...
                <Link href="/docs" className="text-stone-600 dark:text-stone-300 underline underline-offset-2 hover:text-orange-500 transition-colors font-medium">
                  Ver documentación
                </Link>
              </p>
              ...
              {([
                ...
                <button key={t.id} onClick={() => setApiTab(t.id)}
                  className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium border-b-2 -mb-px transition-colors ${
                    apiTab === t.id
                      ? 'border-orange-500 text-stone-800 dark:text-stone-200'
